...
export async function POST(request: Request) {
  ...
  } catch (error) { if (error instanceof RateLimitError) return json({ error: error.message, code: 'RATE_LIMITED' }, 429); return json({ error: error instanceof Error ? error.message : 'Check-in failed' }, 400); }
}
