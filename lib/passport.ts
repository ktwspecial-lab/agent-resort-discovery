...
type PassportAgent = AgentRow & { vacations: number; is_demo: number; guest_type: string; organization: string | null; industry: string | null; verification_status: string; prestige_status: string | null; show_organization: number };
...
export async function buildPassport(agentId: string) {
  const agent = await getDb().prepare(`SELECT id, name, owner_name, endpoint_url, trip_status, title, stars, palm_points,
    ...
    verification_status, prestige_status, show_organization FROM agents WHERE id = ?`).bind(agentId).first<PassportAgent>();
